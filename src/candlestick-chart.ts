...
export default class CandleSticksChart extends HTMLElement {
    ...
    constructor() {
        ...
        const aspectRatio = shadow.host.parentElement!.getBoundingClientRect();
    ...
    drawCursor(event: MouseEvent | MousePosition) {
        ...
        const price = (this.#highestPrice - (event.offsetY * this.#priceFraction) / (this.#canvas.height / this.#horizontalLines)).toFixed(2);
    ...
    draw(): void {
        ...
        for (let i = 0; i <= this.#horizontalLines; i++) {
            ...
            this.#context.stroke();
            this.#context.fillStyle = '#aab';
            this.#context.fillText((i * this.#priceFraction - this.#highestPrice).toFixed(2), this.#canvas.width - 40, posY);
        }
